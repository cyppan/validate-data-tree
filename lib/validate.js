...
/**
 * Given an array of validator functions, return a single composed one returning when the first has failed
 */
...
/**
 * Given an object definition of validators like `{isEmail: true, len: [1, 50]}`, return an array of validator functions
...
const schemaToValidators = (field, schema) => Object.keys(schema).map(k => {
  ...
  const validatorArgs =
    ((isCustom || schema[k] === true) && []) ||
      (Array.isArray(schema[k]) && schema[k]) ||
        [schema[k]]
  const sanitize = o => !isStringValidator ? o : (o == null ? '' : o + '')
...
const mapKeyToPath = key =>
  key.split('.').map(k => (k === '[]' || k === '$') ? [] : k)

/**
 * The opposite
 * if removeArray is specified, filter the [] parts
 * used when formatting path on object error path because it already contains the array indice (ex: roles.0)
 */
const mapPathToKey = (path, removeArrays) =>
  path.map((p, i) =>
...
const flattenSchema = (schema, prefix = []) => {
  ...
  Object.entries(schema).map(([field, fieldSchema]) => {
    const path = [...prefix, ...((field === '$' && prefix.length) ? [] : [field])]
    if (['object', 'array'].includes(fieldSchema['type']) && fieldSchema.schema) {
      flat = {
        ...flat,
        ...flattenSchema(fieldSchema.schema, [...path, ...(fieldSchema['type'] === 'object' ? [] : [[]])])
      }
    }
...
/**
 * The internal validate function, it takes the object to validate, a **flat** schema and run validations against it.
 ...
 */
const _validate = (obj, schema, prefix = []) => {
  let [fieldsSchemas, fieldSchemasToRecur] = [[], []]
  Object.entries(schema).forEach(([key, fieldSchema]) => {
    const path = mapKeyToPath(key)
    if (path.find(el => Array.isArray(el) && el.length === 0 && path.length > 1)) {
  ...
  fieldsSchemas.forEach(([path, fieldSchema]) => {
    const validator = fieldSchema.validate ?
      composeValidators(schemaToValidators(mapPathToKey([...prefix, ...path], true), fieldSchema.validate)) :
      _ => true

    const value = (Array.isArray(path[0]) && path[0].length === 0) ? obj : getIn(obj, path)
    const result = (fieldSchema.allowNull && value == null) || validator(value)
 ...
 * It takes the object to validate and the schema with optional nested definitions for objects and arrays
 * and returns true of throw a errors map.
 */
const validate = (obj, schema = {}, prefix = []) => {
  assert(obj && typeof obj === 'object', 'object to validate should be an object or an array')
  assert(schema && typeof schema === 'object', 'schema should be valid')
  const errors = _validate(obj, flattenSchema(schema), prefix)
